...
本目录记录 AIKB 管理 WebUI（`aikb-web`）的长期架构决策、分阶段实现基线和能力边界。阶段 1、2 与阶段 3 首批受控动作已在 Windows 本机实现并验证；阶段 4A 已完成波次 0～2，共享基础、规则读取/预览以及 `USER_RULES.md` 的原子应用、回滚恢复和任务/审计关联已经落地，尚待波次 3 真实 Windows 与浏览器终验；macOS 仍只保留扩展位置，不代表已经支持。
