...
本目录记录 AIKB 管理 WebUI（`aikb-web`）的长期架构决策、分阶段实现基线和能力边界。阶段 1、2 与阶段 3 首批受控动作已在 Windows 本机实现并验证；阶段 4A 波次 3 已完成隔离 Windows 故障注入、多进程竞争和真实浏览器成功流程，尚待用户明确授权后执行真实 checkout 的等价规则往返，才能形成最终发布结论；macOS 仍只保留扩展位置，不代表已经支持。
