...
阶段 1 已交付 Windows 本地知识读取 MVP；阶段 2 已增加活动 Working State、检查点、双仓安全摘要和审计查询；阶段 3 首批三项只读动作、任务中心、实时事件和 Windows 进程树收敛也已实现。知识、规则与 Git 事实源仍只读，macOS 继续只保留扩展位置，不声称支持。
...
适用于 AIKB 管理 WebUI 从方案确认到分阶段开发的架构和范围控制。阶段 0、阶段 1、阶段 2 与阶段 3 首批只读动作已实现；本文不代表阶段 3 未准入动作、阶段 4 及以后已经实现，也不代表 macOS 已兼容或阶段工期已承诺。
